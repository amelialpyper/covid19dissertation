---
title: "Paramter estimation with uk death data before lockdown"
...
data <- subset(data, location=="United Kingdom"  & date>="2020-03-14" & date<=day_n, select = -c(iso_code, continent, stringency_index, population_density:life_expectancy, new_tests:tests_units)) #display only relevant columns and the UK data only, 
#stating the analysis from the day 10 cumulative deaths are reported, up until the date of UK lockdown order
data
```
```{r}
library(lubridate)
...
if (Sys.getenv("RSTUDIO") == "1" && !nzchar(Sys.getenv("RSTUDIO_TERM")) && 
    ...
    parallel:::setDefaultClusterOptions(setup_strategy = "sequential")} #for chains to run in parallel
...
plot(stan_dens(fit_sir_negbinUK, pars = pars, separate_chains = TRUE), point_est = "mean") 
#plot the density of each parameter output
plot(fit_sir_negbinUK, plotfun = "trace", pars = pars, inc_warmup = FALSE)
#plot of each mcmc chain to see convergence and mixing
plot(fit_sir_negbinUK,pars = pars)
#plot of the spread of each parameter
```
...
guide = "legend")+
  ...
  #plot showing the actual obsevered data against the estmated graph using the bayesian estimates
```
