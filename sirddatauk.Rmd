...
data <- subset(data, location=="United Kingdom"  & date>="2020-03-14" & date<=day_n, select = -c(iso_code, continent, stringency_index, population_density:life_expectancy, new_tests:tests_units)) #display only relavent columns and the UK data only, 
...
# beta <- extract(fit_sir_negbin, "beta")$beta
# hist(beta)
plot(fit_sir_negbinUK,pars = pars)
# plot(fit_sir_negbin,pars = c("beta","gamma","sigma"), show_density = TRUE, ci_level = 0.95, fill_color = "purple")
