...
sirdpois <- function(lbeta,lnu, ltheta, logN, logI0) {
    parms <- c(beta=plogis(lbeta), nu=plogis(lnu), theta=plogis(ltheta))
...
sirdnbin <- function(lbeta,lnu, ltheta, logN, logI0) {
    parms <- c(beta=plogis(lbeta), nu=plogis(lnu), theta=plogis(ltheta))
    x0 <- c(S=exp(logN), I=exp(logI0), R=0, D = 0)
    out <- ode(y=x0, Day, sird, parms)
    SD <- sqrt(sum( (Deaths-out[,5])^2)/length(Day))
...
```{r}
library(ggplot2)
parms <- c(beta=theta1[1], nu = theta1[2], theta = theta1[3])
times <- seq(00,32,0.1)
...
parms <- c(beta=theta2[1], nu = theta2[2], theta = theta2[3])
#setting initial values
