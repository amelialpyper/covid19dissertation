...
require(bbmle)
#  create negative likelihood functions for negbin and poisson distributions. The poisson distribution is given in equation () and the negatuve binomial function in given in equation ().

sirdpois <- function(lbeta,lgamma, lsigma) {
    parms <- c(beta=plogis(lbeta), gamma=plogis(lgamma), sigma=plogis(lsigma))
    x0 <- c(S=N - 1766 , I= 1766, R=30, D = 10)
    out <- ode(y=x0, Day2, sird, parms)
...
sirdnbin <- function(lbeta,lgamma, lsigma, lphi) {
    ...
    out <- ode(y=x0, Day2, sird, parms)
    lphi <- 1/parms["phiinv"]
    -sum(dnbinom(Deathsmle, mu=out[,5], size=lphi, log=TRUE))
