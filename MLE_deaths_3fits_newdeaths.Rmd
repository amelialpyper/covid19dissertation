...
sirdpois <- function(lbeta,lgamma, lsigma) {
    ...
    x0 <- c(S=N - 1766 , I= 1766, R=0, D = 10)
    out <- ode(y=x0, Day2, sird, parms)
    SD <- sqrt(sum( (Deathsmle-out[,5])^2)/length(Day))
...
sirdnbin <- function(lbeta,lgamma, lsigma) {
    ...
    out <- ode(y=x0, Day2, sird, parms)
    SD <- sqrt(sum( (Deathsmle-out[,5])^2)/length(Day))
    -sum(dnbinom(Deathsmle, mu=out[,5], size=SD, log=TRUE))
